    ...
    def _on_client_message(client, self, message):
        if self._message_callback:
            try:
                ...
                _LOGGER.debug(
                    "MQTT message received on topic %s: %s",
                    getattr(message, "topic", "<unknown>"),
                    payload,
                )
                response = json.loads(payload)
                _LOGGER.debug("MQTT decoded response keys: %s", list(response.keys()))
                if "data" in response:
                    _LOGGER.debug("MQTT data payload: %s", response["data"])
                if "data" in response and response["data"]:
                    self._message_callback(response["data"])
            except Exception:
                _LOGGER.exception("Message: can't decode MQTT payload")
                pass
