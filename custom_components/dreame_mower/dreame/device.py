...
class DreameMowerDevice:
    ...
    def __init__(
        ...
        self._last_batch_map_refresh: float = 0  # Last LiDAX batch map config refresh time
        ...
        self._batch_selected_map_name: str | None = None
        self._lidax_live_position: dict[str, Any] | None = None
        self._lidax_live_packet: list[int] | None = None
        self._lidax_position_offset: dict[str, float] | None = None
        self._lidax_previous_candidates: dict[int, int] | None = None
        self._lidax_dock_candidates: dict[int, int] | None = None
        self._lidax_recently_docked_at: float | None = None
    ...
    def _message_callback(self, message):
        ...
        if "method" in message:
            _LOGGER.debug(
                "MQTT method=%s keys=%s payload=%s",
                message.get("method"),
                list(message.keys()),
                message,
            )
            ...
            if message["method"] == "properties_changed" and "params" in message:
                ...
                for param in message["params"]:
                    ...
                    if (
                        param.get("siid") == 1
                        and param.get("piid") == 4
                        and isinstance(value, list)
                    ):
                        if self._handle_lidax_live_position_packet(value):
                    ...
                    if not matched:
                        if (
                            ...
                            param.get("siid") == 2
                            and param.get("piid") == 50
                            and isinstance(value, dict)
                            and value.get("t") == "TASK"
                        ):
                            _LOGGER.debug("LiDAX TASK update received, scheduling batch map refresh: %s", value)
                            self._last_batch_map_refresh = 0
                            if self._refresh_batch_map_data(True):
                                self._property_changed()
                            else:
                                self._handle_lidax_task_hint(value)
                                self.schedule_update(1)
                        _LOGGER.debug(
                            "Unknown MQTT property change siid=%s piid=%s value=%s payload=%s",
                            param.get("siid"),
                            param.get("piid"),
                            param.get("value"),
                            param,
            ...
            elif message["method"] == "event_occured":
                params = message.get("params") or {}
                arguments = params.get("arguments") or []
                needs_batch_refresh = False
                for argument in arguments:
                    if argument.get("piid") == 16 and isinstance(argument.get("value"), str):
                        self._batch_selected_map_name = argument["value"]
                        needs_batch_refresh = True
                        _LOGGER.debug("Batch selected map name hint: %s", self._batch_selected_map_name)
                if needs_batch_refresh:
                    self._last_batch_map_refresh = 0
                    self.schedule_update(1)
            else:
                _LOGGER.debug("Unhandled MQTT method %s payload=%s", message.get("method"), message)
        else:
            _LOGGER.debug("MQTT payload without method: %s", message)

    def _handle_lidax_task_hint(self, value: dict[str, Any]) -> None:
        """Apply a conservative active-map heuristic for LiDAX app-originated map switches.

        The MOVA app sends TASK o=200 when switching maps, but current cloud reads do not expose
        the resulting selected map back to HA. For two-map LiDAX setups we can still keep HA in
        sync by flipping the selected map when that task arrives, as long as the change was not
        triggered moments ago from HA itself.
        """
        if not self._map_manager:
            return

        task_data = value.get("d") if isinstance(value, dict) else None
        if not isinstance(task_data, dict) or task_data.get("o") != 200:
            return

        map_ids = list(self._map_manager.map_list or [])
        if len(map_ids) != 2:
            return

        if self._map_select_time and time.time() - self._map_select_time < 8:
            _LOGGER.debug("Ignoring LiDAX task heuristic because a recent HA map switch is still in flight")
            return

        current_map = self.status.selected_map
        current_map_id = current_map.map_id if current_map else self._map_manager._selected_map_id
        if current_map_id not in map_ids:
            return

        next_map_id = map_ids[0] if map_ids[1] == current_map_id else map_ids[1]
        if next_map_id == current_map_id:
            return

        _LOGGER.info(
            "Applying LiDAX two-map task heuristic: switching selected map from %s to %s",
            current_map_id,
            next_map_id,
        )
        self._map_manager.editor.set_selected_map(next_map_id)
        self._property_changed()

    @staticmethod
    def _lidax_int16(value: list[int], index: int) -> int | None:
        if index + 1 >= len(value):
            return None
        raw = value[index] | (value[index + 1] << 8)
        if raw & 0x8000:
            raw -= 0x10000
        return raw

    @staticmethod
    def _lidax_uint16(value: list[int], index: int) -> int | None:
        if index + 1 >= len(value):
            return None
        return value[index] | (value[index + 1] << 8)

    @staticmethod
    def _lidax_uint24(value: list[int], index: int) -> int | None:
        if index + 2 >= len(value):
            return None
        return value[index] | (value[index + 1] << 8) | (value[index + 2] << 16)
    ...
    @staticmethod
    def _lidax_int16_be(value: list[int], index: int) -> int | None:
        if index + 1 >= len(value):
            return None
        raw = (value[index] << 8) | value[index + 1]
        if raw & 0x8000:
            raw -= 0x10000
        return raw

    @staticmethod
    def _lidax_uint16_be(value: list[int], index: int) -> int | None:
        if index + 1 >= len(value):
            return None
        return (value[index] << 8) | value[index + 1]

    def _lidax_packet_candidates(self, value: list[int]) -> list[dict[str, int]]:
        """Return numeric candidate fields from a LiDAX runtime packet.

        We intentionally expose 16-bit, 32-bit and float32 interpretations in both
        little-endian and big-endian variants. The live position packet is clearly
        structured, but the exact field boundaries are still being reverse engineered
        and assuming only one width/alignment risks missing the real x/y pair entirely.
        """
        candidates: list[dict[str, Any]] = []
        for index in range(0, max(len(value) - 1, 1)):
            signed_le = self._lidax_int16(value, index)
            unsigned_le = self._lidax_uint16(value, index)
            signed_be = self._lidax_int16_be(value, index)
            unsigned_be = self._lidax_uint16_be(value, index)
            if signed_le is not None and unsigned_le is not None:
                candidates.append(
                    {
                        "index": index,
                        "byte_order": "le",
            ...
            if signed_be is not None and unsigned_be is not None:
                candidates.append(
                    {
                        "index": index,
                        "byte_order": "be",
        ...
        for index in range(0, max(len(value) - 3, 1)):
            chunk = bytes(value[index : index + 4])
            if len(chunk) < 4:
                continue
            try:
                signed_le = struct.unpack("<i", chunk)[0]
                unsigned_le = struct.unpack("<I", chunk)[0]
                ...
                if float("-inf") < float_le < float("inf"):
                    candidates.append(
                        {
                            "index": index,
                            "byte_order": "le",
                ...
                pass
            try:
                signed_be = struct.unpack(">i", chunk)[0]
                unsigned_be = struct.unpack(">I", chunk)[0]
                float_be = struct.unpack(">f", chunk)[0]
                candidates.append(
                    {
                        "index": index,
                        "byte_order": "be",
                ...
                if float("-inf") < float_be < float("inf"):
                    candidates.append(
                        {
                            "index": index,
                            "byte_order": "be",
    ...
    def _handle_lidax_live_position_packet(self, value: list[int]) -> bool:
        """Decode LiDAX MQTT runtime position packet from siid=1 piid=4."""
        if len(value) < 5:
            return False

        # LiDAX runtime packet 1.4 carries stable world coordinates at the start of
        # the frame. Earlier guesses based on mid-packet pairs produced placeholder
        # values like 32767 and a frozen marker in HA.
        x = self._lidax_int16(value, 1)
        y = self._lidax_int16(value, 3)
        x_repeat = self._lidax_int16(value, 14)
        mowing_progress_raw = self._lidax_uint16(value, 24)
        mowing_target_area_raw = self._lidax_uint24(value, 26)
        mowing_area_raw = self._lidax_uint24(value, 29)
        if x is None or y is None:
            return False

        candidates = self._lidax_packet_candidates(value)
        previous_candidates = self._lidax_previous_candidates or {}
        candidates_with_delta: list[dict[str, int]] = []
        next_previous_candidates: dict[tuple[int, str, str], float] = {}
        for item in candidates:
            ...
            kind = item.get("kind", "i16")
            key = (index, byte_order, kind)
            next_previous_candidates[key] = numeric_value
            candidate = dict(item)
            if key in previous_candidates:
                candidate["delta"] = numeric_value - previous_candidates[key]
            candidates_with_delta.append(candidate)

        position = {
            "x": x,
            "y": y,
            "a": value[24] if len(value) > 24 and isinstance(value[24], int) else None,
            "x_repeat": x_repeat if isinstance(x_repeat, int) else None,
            "mowing_progress": round(mowing_progress_raw / 100, 2)
            if isinstance(mowing_progress_raw, int)
            else None,
            "mowing_target_area": round(mowing_target_area_raw / 100, 2)
            if isinstance(mowing_target_area_raw, int)
            else None,
            "mowed_area": round(mowing_area_raw / 100, 2) if isinstance(mowing_area_raw, int) else None,
            "raw_packet": list(value),
            "candidates": candidates_with_delta,
        }

        changed = position != self._lidax_live_position
        self._lidax_live_position = position
        self._lidax_live_packet = list(value)
        self._lidax_previous_candidates = next_previous_candidates
        if getattr(self.status, "docked", False):
            self._lidax_dock_candidates = dict(next_previous_candidates)
            self._lidax_recently_docked_at = time.time()
        elif self._lidax_dock_candidates is None and self._lidax_recently_docked_at and time.time() - self._lidax_recently_docked_at < 20:
            # LiDAX does not always emit a runtime 1.4 packet while perfectly docked.
            # Capture the first live packet shortly after undocking as an approximate
            # dock baseline so we can visually compare candidate x/y pairs in HA.
            self._lidax_dock_candidates = dict(next_previous_candidates)
        _LOGGER.debug(
            "Decoded LiDAX live position from MQTT 1.4: x=%s y=%s a=%s x_repeat=%s candidates=%s",
            position["x"],
            position["y"],
            position["a"],
            position["x_repeat"],
    ...
    def _refresh_batch_map_data(self, force: bool = False) -> bool:
        if not self.cloud_connected or self._protocol.cloud is None:
            return False

        now = time.time()
        if not force and now - self._last_batch_map_refresh <= 10:
            return False

        self._last_batch_map_refresh = now
        props = ["TASKID.0", "TASKID.info", "MAP.info", "prop.s_ai_config"]
        props.extend([f"MAP.{index}" for index in range(32)])
        try:
            response = self._protocol.cloud.get_batch_device_datas(props)
        except Exception:
            return False
        ...
        updated = self._update_batch_map_data(response)
    ...
    def _update_batch_map_data(self, response: dict[str, Any] | None) -> bool:
        """Populate saved maps from LiDAX batch config payload when MAP_LIST is unavailable."""
        if not self._map_manager or not isinstance(response, dict):
            return False

        map_length = response.get("MAP.info")
        if map_length is None:
            return False

        map_chunks = []
        for key in sorted(
            [key for key in response if re.fullmatch(r"MAP\.\d+", key)],
            key=lambda item: int(item.split(".")[1]),
        ):
            map_chunks.append(response[key])

        if not map_chunks:
            return False

        try:
            raw_maps = "".join(map_chunks)[: int(map_length)]
            parsed_maps = json.loads(raw_maps)
        except (TypeError, ValueError):
            _LOGGER.debug("Unable to decode LiDAX batch map payload")
            return False

        existing_maps = copy.deepcopy(self._map_manager._saved_map_data)
        updated = False
        for item in parsed_maps:
            try:
                map_info = json.loads(item) if isinstance(item, str) else item
            except (TypeError, ValueError):
                continue

            map_index = map_info.get("mapIndex")
            if map_index is None:
                continue

            map_id = int(map_index)
            map_name = map_info.get("name") or f"Map {map_id + 1}"
            map_data = existing_maps.get(map_id) or MapData()

            if (
                map_data.map_id != map_id
                or map_data.custom_name != map_name
                or map_data.map_name != map_name
                or map_data.saved_map is not True
            ):
                updated = True

            map_data.map_id = map_id
            map_data.custom_name = map_name
        ...
        previous_map_list = list(self._map_manager._map_list)
        previous_selected_map = self._map_manager._selected_map_id
        self._map_manager._saved_map_data = existing_maps
        self._map_manager._refresh_map_list()

        selected_map_id = self._map_manager._selected_map_id
        if selected_map_id not in existing_maps:
            selected_map_id = None

        if selected_map_id is None and self._batch_selected_map_name:
            for map_id, map_data in existing_maps.items():
                if map_data.map_name and map_data.map_name.lower() == self._batch_selected_map_name.lower():
                    selected_map_id = map_id
                    break

        # LiDAX startup state is not reliable enough to guess the active map from
        # TASKID or by falling back to the first saved map. Keep the selected map
        # unset until a confirmed map hint or explicit runtime update arrives.

        self._map_manager._selected_map_id = selected_map_id

        if previous_map_list != list(self._map_manager._map_list) or previous_selected_map != selected_map_id or updated:
            _LOGGER.info(
                "LiDAX batch maps updated: maps=%s selected=%s",
                list(self._map_manager._map_list),
                selected_map_id,
            )
    ...
    def _handle_properties(self, properties) -> bool:
        ...
        for prop in properties:
            ...
            if did is not None:
                try:
                    resolved_property = DreameMowerProperty(int(did))
                except (TypeError, ValueError):
                    resolved_property = None
            ...
            if resolved_property is None:
                ...
                if siid is not None and piid is not None:
                    for candidate, mapping in self.property_mapping.items():
                        if (
                            "aiid" not in mapping
                            and mapping.get("siid") == siid
                            and mapping.get("piid") == piid
                        ):
            ...
            if resolved_property is None:
                _LOGGER.debug(
                    "Unknown mower property id=%s siid=%s piid=%s payload=%s",
                    did,
                    prop.get("siid"),
                    prop.get("piid"),
                    prop,
                ...
                continue
            did = resolved_property.value
            property_name = resolved_property.name
            if prop["code"] == 0 and "value" in prop:
                ...
                if current_value != value:
                    ...
                    if did in self._property_update_callback:
                        _LOGGER.debug("Property %s Callbacks: %s", property_name, self._property_update_callback[did])
        ...
        if info:
            ...
            if self.device_connected and self._protocol.cloud is not None and (not self._ready or not self.available):
                ...
                if self.cloud_connected:
                    ...
                    self._refresh_batch_map_data(True)
    ...
    def update(self, force_request_properties=False) -> None:
        ...
        if self._map_manager and self.cloud_connected:
            self._refresh_batch_map_data()
    ...
    def set_selected_map(self, map_id: int) -> dict[str, Any] | None:
        ...
        if self.status.multi_map:
            ...
            if self._protocol.cloud and self._map_manager and map_id in self.status.map_data_list:
                # LiDAX does not reliably return a command response for map
                # switching, even when the app/device accepts the change. Keep
                # the explicitly requested map as the local HA context so
                # map-scoped sensors do not disappear between cloud refreshes.
                self._map_manager.editor.set_selected_map(map_id)
                self._property_changed()

                # LiDAX switches the active map through a task action rather than the
                # classic UPDATE_MAP_DATA path. This mirrors the official app payload.
                ...
                if response and response.get("code") == 0:
                    self.schedule_update(2, True)
                    return response
                _LOGGER.warning("LiDAX selected map action did not confirm map_id=%s: %s", map_id, response)
                return response
...
class DreameMowerDeviceStatus:
    ...
    def multi_map(self) -> bool:
        ...
        if self._get_property(DreameMowerProperty.MULTI_FLOOR_MAP):
            return True

        if self._map_manager:
            if len(self._map_manager.map_list) > 1:
                return True
            if len(self._map_manager.map_data_list) > 1:
                return True
    ...
    def selected_map(self) -> MapData | None:
        ...
        if self._map_manager:
            ...
            # LiDAX currently exposes saved maps via batch config before the live current-map
            # payload is fully available. Keep the selected saved map usable for HA controls.
            if not self.has_temporary_map and not self.has_new_map:
                return selected_map
            if selected_map.saved_map and selected_map.map_name:
                return selected_map
