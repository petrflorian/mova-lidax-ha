...
SELECTS: tuple[DreameMowerSelectEntityDescription, ...] = (
    ...
    DreameMowerSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            ...
            and len(device.status.map_data_list) > 1
        ),
...
async def async_setup_entry(
    ...
    update_selects = partial(async_update_selects, coordinator, {}, async_add_entities)
    coordinator.async_add_listener(update_selects)
    update_selects()
...
@callback
def async_update_selects(
    coordinator: DreameMowerDataUpdateCoordinator,
    current: dict[str, DreameMowerSelectEntity],
    async_add_entities,
) -> None:
    new_entities = []
    for description in SELECTS:
        key = description.key or description.property_key.name.lower()
        if key not in current and description.exists_fn(description, coordinator.device):
            entity = DreameMowerSelectEntity(coordinator, description)
            current[key] = entity
