...
async def async_setup_entry(
    ...
    platform.async_register_entity_service(
        SERVICE_SELECT_MAP,
        {
            ...
            vol.Optional(INPUT_MAP_INDEX): cv.positive_int,
            vol.Optional(INPUT_MAP_NAME): cv.string,
        },
...
class DreameMower(DreameMowerEntity, LawnMowerEntity):
    ...
    def available(self) -> bool:
        ...
        return self.device.device_connected
    ...
    async def async_select_map(self, map_id=None, map_index=None, map_name=None) -> None:
        """Switch selected map."""
        if map_id is None and map_index is None and not map_name:
            raise HomeAssistantError("One of map_id, map_index or map_name is required")

        if map_id is None:
            if map_index is not None:
                map_data = self.device.get_map(map_index)
                if map_data is not None:
                    map_id = map_data.map_id
            elif map_name:
                for current_map in self.device.status.map_data_list.values():
                    if current_map.map_name == map_name:
                        map_id = current_map.map_id
                        break

        if map_id is None:
            raise HomeAssistantError("Unable to resolve selected map")
